...
public class InsightEngine {
    ...
    public List<InsightDTO> generateInsights(
        ...
        Map<String, Double> currentExp =
                expensesByCategory(transactions, current);

        Map<String, Double> previousExp =
                expensesByCategory(transactions, previous);
    ...
    public HealthScoreDTO computeHealthScore(
        ...
        double budgetAdherence =
                budgetAdherenceScore(transactions, budgets, current);

        double spendingStability =
                stabilityScore(transactions, current, previous);

        double overspendPenalty =
                overspendingPenalty(transactions, budgets, current);

        double raw =
                (savingsRatio * 100.0 * SAVINGS_WEIGHT)
    ...
    private List<InsightDTO> overspendingInsights(
        ...
        for (Map.Entry<String, Double> entry : current.entrySet()) {
            ...
            double previousValue =
                    previous.getOrDefault(category, 0.0);

            if (previousValue <= 0) continue;

            double percent =
                    percentChange(previousValue, currentValue);

            if (percent >= OVERSPEND_THRESHOLD_PCT) {

                String severity =
                        percent >= OVERSPEND_DANGER_PCT
                                ? "danger"
                                : "warning";
    ...
    private List<InsightDTO> budgetWarnings(
        ...
        for (Budget budget : budgets) {
            ...
            if (budget.getLimitAmount() == null) continue;
    ...
    private double budgetAdherenceScore(
        ...
        if (budgets.isEmpty()) return 70.0;
    ...
    private double stabilityScore(
        ...
        double currentExpenses =
                totalExpenses(transactions, current);

        double previousExpenses =
                totalExpenses(transactions, previous);

        if (previousExpenses <= 0) return 70.0;
