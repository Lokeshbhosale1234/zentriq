...
// LAYOUT NOTE
// ─────────────────────────────────────────────────────────────────────────────
// Trimmed from 7 stacked sections down to 4 to match the reference design's
// information density (Efferd-style: stat row → 1 chart row → 1 detail row).
// Removed sections were either demo-only placeholders or redundant with data
// already shown elsewhere on this same page:
//   - QuickActions      → redundant with the "+ Add" button already in Topbar
//   - GoalsWidget        → static demo data, not backend-driven
//   - UpcomingBills      → static demo data, not backend-driven
//   - MonthlyBarChart    → duplicate of BalanceLineChart (same income/expense data)
//   - Investments banner → static "coming soon" placeholder
// None of these removed components are deleted from the codebase — only
// unmounted from the Dashboard route — so nothing in routing/API breaks and
// they can be re-added to any page later if needed.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ padding: '8px 14px', borderRadius: 10, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.18)', display: 'flex', alignItems: 'center', gap: 7 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }} className="lg:grid-cols-4">
        <StatCard title="Monthly Income"   value={analytics?.totalIncome ?? 0}  color="green" icon={<IconIncome />}  loading={aLoading} prefix="₹" subtitle="This month's total" />
        <StatCard title="Monthly Expenses" value={analytics?.totalExpense ?? 0} color="red"   icon={<IconExpense />} loading={aLoading} prefix="₹" subtitle="Total spent this month" />
        <StatCard title="Net Savings"      value={savings}                     color="mono"  icon={<IconBalance />} loading={aLoading} prefix="₹" subtitle={`Savings rate: ${savingsRate}%`} />
        <StatCard title="Transactions"     value={analytics?.totalTransactions ?? 0} color="mono" icon={<IconCount />} loading={aLoading} prefix="" subtitle="Total this month" />
      </div>
