  ...
  return (
    <div style={{ marginTop: 7 }}>
      <div style={{ display: 'flex', gap: 4, marginBottom: 5 }}>
        {[1,2,3,4].map(i => <div key={i} style={{ flex: 1, height: 2, borderRadius: 99, background: i <= score ? colors[score] : 'rgba(255,255,255,0.07)', transition: 'background 0.2s' }} />)}
      </div>
...
export default function SignupPage() {
  ...
  const label = (text) => ({ display: 'block', fontSize: 10, fontWeight: 700, color: '#444444', marginBottom: 7, textTransform: 'uppercase', letterSpacing: '0.08em' })
    ...
    <div style={{ minHeight: '100dvh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <p style={{ fontSize: 11, color: '#2a2a2a', textAlign: 'center', marginTop: 18, lineHeight: 1.6 }}>
          ...
          <span style={{ color: '#3a3a3a', cursor: 'pointer' }}>Terms of Service</span>{' '}and{' '}
          <span style={{ color: '#3a3a3a', cursor: 'pointer' }}>Privacy Policy</span>.
        </p>
      </div>
    </div>
  )
