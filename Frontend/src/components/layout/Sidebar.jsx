...
/* ─────────────────────────────────────────────────────────────────────────────
   ArvexaIcon — inline SVG A-monogram
   ─────────────────────────────────────────────────────────────────────────────
   Pure white paths on transparent background. NO background rect whatsoever.
   The split crossbar gap naturally reads as an upward arrow in negative space.
   This replaces the old dot-grid AND the downloaded SVG file (which had a
   black <rect> causing the dark box / embedded-image look in the UI).
   ─────────────────────────────────────────────────────────────────────────────*/
export const ArvexaIcon = ({ size = 28 }) => (
  ...
  >
    {/* Left leg */}
    <polygon points="10,86 36,14 46,14 20,86" fill="#ffffff" />
    {/* Right leg */}
    <polygon points="90,86 64,14 54,14 80,86" fill="#ffffff" />
    {/* Crossbar left — gap between the two halves IS the arrow */}
    <rect x="20" y="52" width="22" height="11" rx="1" fill="#ffffff" />
    {/* Crossbar right */}
    <rect x="58" y="52" width="22" height="11" rx="1" fill="#ffffff" />
  </svg>
)
...
/* collapsed is now received as a PROP from Layout — no internal state */
    ...
    <>
      ...
      >
        {/* ── Logo row — no toggle button here ──────────────────────
            The sidebar toggle is now in the Topbar (Efferd pattern):
            a panel icon in the content header, not inside the sidebar.
            This removes the awkward arrow button the user complained about.
        ──────────────────────────────────────────────────────────── */}
        <div style={{
              ...
              >
                ...
                {!collapsed && (
                  <>
                    ...
                    {item.badge && (
                      <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 5px', borderRadius: 99, background: 'rgba(255,255,255,0.1)', color: '#888888' }}>
                        {item.badge}
                      </span>
                    )}
                  </>
        ...
        >
          {showAccountMenu && (
            <div className="account-popover" role="menu">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px 10px', borderBottom: '1px solid var(--border-subtle)', marginBottom: 4 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, background: '#222222', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#ffffff' }}>
                  {initials}
                </div>
              ...
              <div className="account-popover-item" role="menuitem"><IconUser /> Account</div>
              <div className="account-popover-item" role="menuitem"><IconBilling /> Billing</div>
              <div className="account-popover-item" role="menuitem"><IconBell /> Notifications</div>
              <div style={{ height: 1, background: 'var(--border-subtle)', margin: '4px 6px' }} />
              <div className="account-popover-item danger" role="menuitem"
          ...
          >
            <div style={{ width: 30, height: 30, borderRadius: 7, flexShrink: 0, background: '#222222', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#ffffff' }}>
              {initials}
            </div>
            {!collapsed && (
              <>
                <div style={{ flex: 1, overflow: 'hidden', minWidth: 0 }}>
                  <p style={{ fontSize: 12, fontWeight: 600, color: '#ffffff', lineHeight: 1.3 }} className="truncate">{user?.name || 'User'}</p>
                  <p style={{ fontSize: 10, color: '#555555' }} className="truncate">{user?.email || ''}</p>
                </div>
