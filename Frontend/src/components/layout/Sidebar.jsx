...
const PLACEHOLDER_ITEMS = [
  {
    to: '/ai', label: 'AI Insights', badge: 'Soon',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  {
    to: '/payment', label: 'Payments', badge: 'Soon',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-5" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <button onClick={() => setCollapsed(!collapsed)}
            className="ml-auto flex-shrink-0 w-6 h-6 rounded flex items-center justify-center hidden lg:flex"
          ...
          {/* Mobile close */}
          <button onClick={onMobileClose} className="ml-auto lg:hidden flex-shrink-0 w-6 h-6 rounded flex items-center justify-center"
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          {!collapsed && (
            <p className="px-3 mb-2 text-xs font-600 uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Main</p>
          )}
          {NAV_ITEMS.map(item => (
            <NavLink
              key={item.to}
          ...
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div className="overflow-hidden">
                <p className="text-sm font-500 truncate" style={{ color: 'var(--text-primary)' }}>FinFlow</p>
                <p className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>Dashboard</p>
              </div>
            )}
          </div>
        </div>
